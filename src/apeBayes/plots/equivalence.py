...
def plot_equivalence_matrix_with_dendrogram(
    ...
    *,
    ...
    axes[1].set_title(rf"Epistemic equivalence ($\alpha={alpha}$)", fontsize=8)
...
def plot_equivalence_bars_plus_sweep(
    ...
    *,
    ...
    # Left: bar chart at fixed alpha
    df_bar = equiv_df.copy()
    labels_list = order_config_labels(df_bar[label_col].astype(str).tolist())
    df_bar = df_bar.set_index(label_col).loc[labels_list].reset_index()
    ...
    ax.set_ylabel(rf"$P(|\Delta\mu| < {alpha}\,\sigma_{{\mathrm{{run}}}})$")
    ax.set_ylim(0, 1.05)
    ax.set_title(rf"Equivalence at $\alpha = {alpha}$", fontsize=8)
    ...
    ax2.legend(fontsize=5, ncol=3, loc="lower right")
    ax2.set_title("Equivalence probability sweep", fontsize=8)
