...
def plot_standardized_bias(
    ...
    *,
    ...
    """Layered standardised-bias plot with violins, raw dots, X markers,
    and posterior credible intervals with cap-handle error bars.
    ...
    beta_draws : (S, n_configs) array, optional
        ...
        When provided, symmetric violin KDEs are drawn behind the
    ...
    if station_subplots and station_col in bias_df.columns:
        ...
        for idx, sta in enumerate(stations):
            ...
            axes[idx].set_title(str(sta), fontsize=8)
            if idx > 0:
                axes[idx].set_ylabel("")
...
def _draw_bias_panel(
    ...
    *,
    ...
    Layers (back to front):
      0  Grey aleatory band |beta| < 1
      1  Symmetric violin KDEs of posterior beta (if beta_draws provided)
    ...
    if beta_draws is not None and beta_labels is not None:
        ...
        violin_hw = 0.35  # half-width of the violin in y-axis units
        for i, bl in enumerate(beta_labels):
            ...
            # Symmetric violin mirrored about the row centre
            ax.fill_between(
                x_grid, yi - density, yi + density,
                alpha=0.18, color=PALETTE[6], zorder=1,
                label=("posterior density" if i == 0 else None),
            )
            ax.plot(x_grid, yi - density, lw=0.5, color=PALETTE[6],
                    alpha=0.5, zorder=1)
            ax.plot(x_grid, yi + density, lw=0.5, color=PALETTE[6],
                    alpha=0.5, zorder=1)
