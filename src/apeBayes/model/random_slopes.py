...
class RandomSlopesModel:
    ...
    def sigma_GM(self, post: PosteriorAccessor) -> np.ndarray:
        """σ_GM = σ_src for random-slopes models (v4–v7).

        The per-Case loading λ_case multiplies the run effect but is a
        configuration-dependent rescaling of the *same* source layer; it
        is absorbed into the effective slope, not treated as an independent
        aleatory component. No σ_inter is present at this tier either.
        See ``uncertanty_measures.md`` §7.
        """
